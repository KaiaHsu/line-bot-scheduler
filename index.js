    ...
    try {
      ...
      if (event.message.type === 'text') {
        ...
        if (session.step === 'deleteGroup') {
          ...
          const success = await groupStore.deleteGroupByIndex(index)
          sessionStore.clear(userId)
          return client.replyMessage(replyToken, {
            type: 'text',
            text: success
        ...
        if (userMessage === '排程推播' && !session.step) {
          const savedGroups = await groupStore.getAllGroups()
          session.step = 'group'
          sessionStore.set(userId, session)
        ...
        if (session.step === 'group') {
          if (/^\d+$/.test(userMessage)) {
            const group = await groupStore.getGroupByIndex(Number(userMessage.trim()))
            if (group) {
