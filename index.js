...
async function safeGetSession(userId) {
  ...
  if (
    session.lastActive &&
    Date.now() - session.lastActive > SESSION_TIMEOUT
  ) {
      ...
      try {
        ...
        if (event.message.type === 'text') {
          ...
          if (
            ...
            const buf = await client.getMessageContent(
              event.message.id
            )
            const arr = []
            ...
            return client.replyMessage(replyToken, {
              ...
              }（第 ${session.mediaList.length} 支），完成請輸入「完成」`,
            })
          }
